...
#pragma clang diagnostic pop
    ...
    if (sender.tag == 4) {
        ...
        NSMethodSignature *methodSignature = [self methodSignatureForSelector:@selector(method3:)];
        NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:methodSignature];
        [invocation setTarget:_helper];
        [invocation setSelector:@selector(method3:)];//虽然已经对方法method3:进行签名了,但这个设置必须实现.
...
void functionForMethod(id self, SEL _cmd) {//方法的实现IMP
    NSLog(@"方法0");
}
